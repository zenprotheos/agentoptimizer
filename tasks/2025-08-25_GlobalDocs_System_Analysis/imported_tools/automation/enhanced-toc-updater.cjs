...
class EnhancedTOCUpdater {
  ...
  async getAllMarkdownFiles() {
    return glob.sync(`${this.baseDir}/**/*.md`);
  }
  ...
  updateTOC(content) {
    ...
    const newTOCSection = [
      ...
      '- **ALWAYS update the Table of Contents above when adding new sections or major changes**',
      '- **Keep TOC links synchronized with actual section headers**',
...
Usage:
  node tools/automation/enhanced-toc-updater.cjs [options]
